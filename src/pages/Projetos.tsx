...
export default function Projetos() {
  ...
  function adicionarEnvolvido() {
    ...
    const jaExiste = form.envolvidos.some(
      (e) => e.pessoaId === novoEnvPessoa && e.papel === novoEnvPapel,
    )
    if (jaExiste) return
  ...
  function salvar(e: React.FormEvent) {
    ...
    if (editId !== null) {
      setProjetos((lista) => lista.map((p) => (p.id === editId ? { id: editId, ...form } : p)))
    } else {
      const proximoId = projetos.reduce((m, p) => Math.max(m, p.id), 0) + 1
      setProjetos((lista) => [...lista, { id: proximoId, ...form }])
    }
    setModo('lista')
  }
  ...
  if (modo === 'lista') {
    return (
      <div className="page">
        <PageHeader
          title="Projetos"
          subtitle="Lista de projetos com busca, edição e (em breve) filtros e exportação"
        />
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        subtitle="Preencha os dados do projeto e associe os envolvidos por papel"
      />
      ...
      <form onSubmit={salvar}>
        ...
        <div className="card form-card">
          ...
          <p className="muted">
            Associe pessoas por papel. A mesma pessoa pode ter vários papéis.
          </p>
